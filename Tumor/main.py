...
import time
import os
from get_model import Resnet18 as get_model_f
simu_num = 0
...
file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "Dataset/Tumor_dataset/")
save_path = "" + name_of_save

print("loading Tumor")
nameall=["glioma","meningioma","notumor","pituitary"]
...
def trans_x(x, device="cpu"):
    mm = torch.tensor([0.1868, 0.1868, 0.1869]).reshape(1, 3, 1, 1).to(device)
    ss = torch.tensor([0.2020, 0.2020, 0.2021]).reshape(1, 3, 1, 1).to(device)
    x = x.float()
    x = x.to(device)
    x = (x / 255 - mm) / ss
    return x
...
plot_compare(loaded_list=resultout, Budget=300,batch_size=20,n0=100, lbub=None,
             indexuse=8, stdstart=0, usestd=False, plotfull=False, xlabel="Budget",
             ylabel="Accuracy", title="Tumor",
