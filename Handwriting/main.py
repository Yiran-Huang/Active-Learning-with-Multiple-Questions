...
import time
import os
from get_model import Logistic as get_model_f
simu_num = 0
device="cpu"
...
file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "Dataset/Handwriting_dataset/")
save_path = "" + name_of_save
csv_file_path = file_path + 'data.csv'
import pandas as pd
dataall = pd.read_csv(csv_file_path)
labelall= dataall.iloc[:, 0]
dataall = dataall.iloc[:, 1:]
dataall = torch.tensor(np.array(dataall) / 16, dtype=torch.float32)
labels_category = labelall.astype('category')
labelall = labels_category.cat.codes
labelall = torch.tensor(np.array(labelall), dtype=torch.int64)
dataall = (dataall-dataall.mean(dim=0))/(dataall.std(dim=0)+1e-5)
...
plot_compare(loaded_list=resultout, Budget=100, batch_size=5, n0=150, lbub=None,
             indexuse=8, stdstart=0, usestd=False, plotfull=False, xlabel="Budget",
             ylabel="Accuracy", title="Handwriting",
