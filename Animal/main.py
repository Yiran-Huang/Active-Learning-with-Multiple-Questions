...
import time
import os
from get_model import Resnet50_transfer as get_model_f
simu_num = 0
...
file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "Dataset/Animal_dataset/")
save_path = "" + name_of_save

print("loading Animal")
import numpy as np
import torch
nameall=["cane","cavallo","elefante","farfalla","gallina","gatto","mucca","pecora","ragno","scoiattolo"]

dataall=torch.empty((0,3,224,224),dtype=torch.uint8)
labelall = torch.empty(0,dtype=torch.int64)
count=0
for namethis in nameall:
    data = np.load(file_path+namethis+"x.npy")
    data=torch.tensor(data,dtype=torch.uint8)
    dataall = torch.cat((dataall,data),0)
    labelthis=torch.full((data.shape[0],), count, dtype=torch.int64)
    labelall = torch.cat((labelall,labelthis))
    count+=1
    del labelthis,data
print("Animal loaded")
...
def trans_x(x, device="cpu"):
    mm = torch.tensor([0.5183, 0.5010, 0.4131]).reshape(1, 3, 1, 1).to(device)
    ss = torch.tensor([0.2681, 0.2632, 0.2808]).reshape(1, 3, 1, 1).to(device)
    x = x.float()
    x = x.to(device)
    x = (x / 255 - mm) / ss
    return x
...
for loop in range(loop_times):
    ...
    Nall=len(dataall)
    indices = torch.randperm(Nall)
    train_size = int(0.8 * Nall)
    train_idx = indices[:train_size]
    test_idx = indices[train_size:]
    del indices
    x_all = dataall[train_idx]
    y_all = labelall[train_idx]
    x_test = dataall[test_idx]
    y_test = labelall[test_idx]
...
plot_compare(loaded_list=resultout, Budget=500,batch_size=50,n0=100, lbub=None,
             indexuse=8, stdstart=0, usestd=False, plotfull=False, xlabel="Budget",
             ylabel="Accuracy", title="Animal",
